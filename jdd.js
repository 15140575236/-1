...
var jdd = {
    ...
    /**
    *remove the last index "null" in the data array
    */

    removeLastIndex: function(val) {
        var newArr=[];
    ...
    exportReport: function() {
            ...
            console.log("jdd.diffs.path1:" + JSON.stringify(jdd.diffs[0].path2.path));

            for(var i=0;i<jdd.diffs.length;i++){
//                var path_missing = JSON.stringify(jdd.diffs[i].msg);
                if("Both sides should be equal strings" === JSON.stringify(jdd.diffs[i].msg)){
                    data1.push(JSON.stringify(jdd.diffs[i].msg));
                }else{
                    data2.push(jdd.diffs[i].msg);
                }
            }
            console.log("data2+ :" + data2);
            for(var i in data2){
                if(data2[i].indexOf("Both sides should be equal strings") ==-1)
                data3.push(data2[i]);
             }
             ...
             var arraright = [];
             for(var i in data3){
                if(data3[i].indexOf('on the left side')==-1){
                    arraright.push(data3[i]);
                    arraleft.push("&#10004");
                }
             }
             for(var i in data3){
                 if(data3[i].indexOf('on the right side')==-1){
                    arraleft.push(data3[i]);
                    arraright.push("&#10004");
                  }
             }
             console.log(arraleft);
             console.log(arraright);

             var dataLeft = [];
             var dataRight = [];
             dataLeft = $('pre.left div.codeLine').children('.eq').text().trim().replace(/\s*/g,"");
             dataLeft = dataLeft.split(",");
             console.log("dataLeft:"+dataLeft);

             dataRight = $('pre.right div.codeLine').children('.eq').text().trim().replace(/\s*/g,"");
             dataRight = dataRight.split(",");

           jdd.tableToExcel(dataLeft,dataRight,arraleft,arraright) ;
    ...
    /**
     * table to excel report
     */
     tableToExcel: function(data1,data2,data3,data4) {

              var optionsleft=$("#dbleft option:selected").val();
//              console.log(apileft);
              var optionsright=$("#dbright option:selected").val();
              var str="<table style='font-size:26px;background:#fff;margin:45px;width:480px;border-collapse:collapse;text-align:left;'>";
              str = str + "<thead><tr><th style='font-size:30px;font-weight:normal;color:#039;padding:10px 8px;border-bottom:2px solid #6678b1;border:1px solid #6678b1;'> Diffs Exist in "+optionsleft+"</th><th style='font-size:30px;font-weight:normal;color:#039;padding:10px 8px;border-bottom:2px solid #6678b1;border:1px solid #6678b1;'> Diffs Exist in "+optionsright+"</th></tr><thead>";
              str = str + "<tbody>";
              if(data1.length === data2.length){
                     for(var i=0;i<data1.length;i++){
                        if(data1[i] === ""&& data2[i] === ""){
                            ...
                            str = str + "<tr style='color:#009;padding:9px 8px 0px 8px;'><td style='color:#669;padding:9px 8px 0px 8px;border:1px solid #6678b1;'>" + data1[i] +"</td><td style='color:#669;padding:9px 8px 0px 8px;border:1px solid #6678b1;'>"+data2[i]+"</td></tr>";
                        }
                     }
               }

               var str= str + "</tbody></table>";
               str = str + "<table style='font-size:26px;background:#fff;margin:45px;width:520px;border-collapse:collapse;text-align:center;'>";
                             str = str + "<thead><tr><th style='font-size:30px;font-weight:normal;color:#039;padding:10px 8px;border-bottom:2px solid #6678b1;border:1px solid #6678b1;'> Missing Exist in "+optionsleft+"</th><th style='font-size:30px;font-weight:normal;color:#039;padding:10px 8px;border-bottom:2px solid #6678b1;border:1px solid #6678b1;'> Missing Exist in "+optionsright+"</th></tr><thead>";
                             str = str + "<tbody>";
                             if(data3.length === data4.length){
                                    for(var i=0;i<data3.length;i++){
                                       str = str + "<tr style='color:#009;padding:9px 8px 0px 8px;'><td style='color:#669;padding:9px 8px 0px 8px;border:1px solid #6678b1;'>" + data3[i] +"</td><td style='color:#669;padding:9px 8px 0px 8px;border:1px solid #6678b1;'>"+data4[i]+"</td></tr>";
                                    }
                              }
               var str= str + "</tbody></table>";
              //encodeURIComponent解决中文乱码
              var uri = 'data:text/html;charset=utf-8,\ufeff' + encodeURIComponent(str);
              //通过创建a标签实现
              var link = document.createElement("a");
              link.href = uri;
              //对下载的文件命名
              link.download =  "comapre.html";
    ...
    generateReport: function () {
        ...
        var ex = $('<div><button class="ex" id="ex"  >Export Report</button></div>');
        report.append(ex);
        ex.click(function(){
             jdd.exportReport();
         });
